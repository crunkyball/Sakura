...
SaResource<T>* SaResourceManager::LoadResource(const char* pFileName)
{
    ...
    if (!pResource)
    {
        ...
        strncpy(pResource->m_debugName, pFileName, SaResource<T>::DEBUG_NAME_SIZE-1);
...
void SaResourceManager::ResourceManagerThread_Load<SaEffect>(const SaResourceJob* pJob)
{
    ...
    if (pEffect)
    {
        ...
        sprintf(shaderFileName, "%s_v%s", pJob->GetFileName(), SaEffect_Platform::FILE_EXTENSION);
        vertexShaderLoader.Load(shaderFileName, NO_CALLBACK, true, true);

        if (vertexShaderLoader.Succeeded())
        {
            sprintf(shaderFileName, "%s_f%s", pJob->GetFileName(), SaEffect_Platform::FILE_EXTENSION);
